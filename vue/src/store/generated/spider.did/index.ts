import { Client, registry, MissingWalletError } from 'spider-client-ts'

import { Strings } from "spider-client-ts/spider.did/types"
import { DIDDocument } from "spider-client-ts/spider.did/types"
import { VerificationMethod } from "spider-client-ts/spider.did/types"
import { VerificationRelationship } from "spider-client-ts/spider.did/types"
import { Service } from "spider-client-ts/spider.did/types"
import { DIDDocumentWithSeq } from "spider-client-ts/spider.did/types"
import { DataWithSeq } from "spider-client-ts/spider.did/types"
import { Params } from "spider-client-ts/spider.did/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: spider.did initialized!')
		...
		async QueryDID({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SpiderDid.query.queryDId( key.did_base64)).data
		...
		async sendMsgCreateDID({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SpiderDid.tx.sendMsgCreateDID({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgCreateDID({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SpiderDid.tx.msgCreateDID({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateDID:Init Could not initialize signing client. Wallet is required.')
