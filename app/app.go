...
import (
	...
	didmodule "spider/x/did"
	didcrypto "spider/x/did/client/cli/crypto"
	didmodulekeeper "spider/x/did/keeper"
	didmoduletypes "spider/x/did/types"

	// this line is used by starport scaffolding # stargate/app/moduleImport

	appparams "spider/app/params"
	"spider/docs"
)
...
func createDID(ctx client.Context) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 	http.Error(w,err.Error(),500)
		// }
		// clientCtx = ctx.WithSkipConfirmation(true)
		// clientCtx = ctx.WithFrom(record.Name)
		// clientCtx = ctx.WithFromAddress(sdkAddress)
		// clientCtx = ctx.WithFromName(record.Name)
		...
		// // w.Write([]byte(ctx.From))
		// w.Write([]byte(ctx.FromAddress))
		// w.Write([]byte(ctx.FromName))

		// keeper,goCtx := DidKeeper()
		...
		// seq := didmoduletypes.InitialSequence
		...
		// docWithSeq := didmoduletypes.NewDIDDocumentWithSeq(msg.Document, uint64(seq))
		...
		// grpcResp, err := txClient.BroadcastTx(ctx,)

		// w.Write([]byte(InpData.Address))
		// w.Write([]byte(txResp.Tx.Value))
		// err = tx.GenerateOrBroadcastTxCLI(ctx, nil, msg)
...
func VerifyDIDOwnership(signData *didmoduletypes.DIDDocument, seq uint64, doc *didmoduletypes.DIDDocument, verificationMethodID string, sig []byte) (uint64, error) {
	verificationMethod, ok := doc.VerificationMethodFrom(doc.Authentications, verificationMethodID)
	if !ok {
		return 0, sdkerrors.Wrapf(didmoduletypes.ErrVerificationMethodIDNotFound, "VerificationMethodId: %s", verificationMethodID)
	}
	...
	//       But, need to support other key didmoduletypes (according to verificationMethod.Type).
	if verificationMethod.Type != didmoduletypes.ES256K_2019 && verificationMethod.Type != didmoduletypes.ES256K_2018 {
		return 0, sdkerrors.Wrapf(didmoduletypes.ErrVerificationMethodKeyTypeNotImplemented, "VerificationMethod: %v", verificationMethod.Type)
	}
	pubKeySecp256k1, err := secp256k1util.PubKeyFromBase58(verificationMethod.PublicKeyBase58)
	if err != nil {
		return 0, sdkerrors.Wrapf(didmoduletypes.ErrInvalidSecp256k1PublicKey, "PublicKey: %v", verificationMethod.PublicKeyBase58)
	}
	newSeq, ok := didmoduletypes.Verify(sig, signData, seq, pubKeySecp256k1)
	if !ok {
		return 0, didmoduletypes.ErrSigVerificationFailed
	}
	return newSeq, nil
}
